...
export default function SettingsPage({ javaSetupBusy = false, javaSetupStatus, settings, onSave, onSetupJava, onRenameGameDirectory, autoUpdateStatus, importer, onSetupGuide, setupGuideDisabled }: SettingsPageProps) {
  const ui = useUiText();
  const [section,setSection]=useState<SettingsSection|"import">("general");
  ...
  return (
    <div className="page-stack">
      ...
      <nav className="settings-tabs" aria-label={de?"Einstellungsbereiche":ui("Settings categories")}>{([['general',de?'Allgemein':ui("General")],['minecraft','Minecraft & Java'],['downloads','Downloads'],['import','Import'],['advanced',de?'Erweitert':ui("Advanced")]] as const).map(([id,label])=><Button key={id} variant={section===id?'primary':'secondary'} onClick={()=>setSection(id)}>{label}</Button>)}</nav>
      {section!=="import"&&<section className="settings-grid"><SettingsForm section={section} javaSetupBusy={javaSetupBusy} javaSetupStatus={javaSetupStatus} settings={settings} onSave={onSave} onSetupJava={onSetupJava} onRenameGameDirectory={onRenameGameDirectory} autoUpdateStatus={autoUpdateStatus} /></section>}
